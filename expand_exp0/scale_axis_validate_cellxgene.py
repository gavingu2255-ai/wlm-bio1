"""
scale_axis_validate_cellxgene.py
===========================================================================
Topology-Induced Scale Axis -- Cross-Tissue Generalization Validation
(CZ CELLxGENE versions: Tabula Sapiens Liver / Lung, hash-named h5ad files)

This is a sibling script to scale_axis_validate_tissue.py, adapted for the
CZ CELLxGENE download versions of the same Tabula Sapiens tissues. These
files use different field names than the figshare "named" versions:

    CZ CELLxGENE field           figshare "named" field
    ---------------------------  -----------------------
    var['feature_name']          var['gene_symbol']
    obs['cell_type']             obs['cell_ontology_class']
    obs['assay']                 obs['method']

Both scripts produce directly comparable output (same statistics, same
figure layout) so that results from the two independently-processed
versions of the same tissue can be cross-checked against each other.

Data: CZ CELLxGENE hash-named h5ad files
      (e.g. a4b3a49e-062b-4e3f-8915-02f40607651f.h5ad for Liver,
            fddf43a3-b7c8-49e9-ac0a-5e24b390849f.h5ad for Lung)
...
# 0. CLI arguments
# ===========================================================================
parser = argparse.ArgumentParser(description="Scale axis validation on a CZ CELLxGENE tissue h5ad")
parser.add_argument("--file", type=str, required=True,
                     help="Path to h5ad file, e.g. a4b3a49e-....h5ad")
...
# ===========================================================================
# 1. Marker gene sets by tissue (identical to scale_axis_validate_tissue.py,
#    kept here so this script is fully standalone)
# ===========================================================================
TISSUE_MARKERS = {
    "liver": {
...
# ===========================================================================
# 2. Load data -- CZ CELLxGENE field mapping
# ===========================================================================
...
# --- Field mapping for CZ CELLxGENE h5ad ---
# CZ CELLxGENE conveniently provides both an ontology-term-ID column
# (cell_type_ontology_term_id, e.g. "CL:0000775") and a plain-English
# column (cell_type, e.g. "macrophage"). We use the plain-English one.
if "cell_type" not in adata.obs.columns:
    raise KeyError("Expected obs['cell_type'] not found. "
                   "Is this the CZ CELLxGENE hash-named h5ad, not the figshare 'named' version? "
                   f"Available obs columns: {adata.obs.columns.tolist()}")
if "feature_name" not in adata.var.columns:
    raise KeyError("Expected var['feature_name'] not found. "
                   f"Available var columns: {adata.var.columns.tolist()}")

adata.obs["cell_type_label"] = adata.obs["cell_type"].astype(str)
# feature_name is used directly below via adata.var["feature_name"]
...
assay = np.array(adata.obs["assay"]) if "assay" in adata.obs.columns else None
...
if assay is not None:
    print(f"  Assay platforms present: {np.unique(assay).tolist()}")
...
# ===========================================================================
# 4. Cell-type signal vs eigenvalue index
# ===========================================================================
...
# ===========================================================================
# 6. Gradient flow: cell-type separation over time
# ===========================================================================
...
fig.suptitle(
    f"Topology-Induced Scale Axis -- {TISSUE.capitalize()} Validation (CZ CELLxGENE)\n"
    f"({n_cells} cells, {n_genes} genes, {len(unique_ct)} real cell types via cell_type)",
...
fig_path_pdf = f"real_figures/fig_{TISSUE}_cellxgene_validation.pdf"
fig_path_png = f"real_figures/fig_{TISSUE}_cellxgene_validation.png"
...
# ===========================================================================
results = {
    "data": f"{args.file} (CZ CELLxGENE) -- tissue={TISSUE} ({n_cells} cells, {n_genes} genes, "
...
results_path = f"real_results/{TISSUE}_cellxgene_results.json"
with open(results_path, "w") as f:
    json.dump(results, f, indent=2)

print(f"\n{'='*65}")
print(f"  {TISSUE.upper()} VALIDATION SUMMARY (CZ CELLxGENE)")
print(f"{'='*65}")
